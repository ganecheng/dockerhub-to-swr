...
| 组件 | 版本 | 引用文件 | 说明 |
|------|------|----------|------|
| Docker CE | `28.5.2` | `gitea-runner-ubuntu/Dockerfile` (静态二进制), 各 workflow (`docker/setup-docker-action@v5` with `version: 'v28.5.2'`) | 容器引擎 |
| Docker CE (apt) | apt 默认最新 | `k3s/Dockerfile` | 通过 Docker 官方 APT 源安装 |
| containerd.io | apt 默认最新 | `k3s/Dockerfile` | Docker CE 运行依赖 |
| k3s | `v1.34.9+k3s1` | `k3s/Dockerfile` | 轻量级 Kubernetes 发行版 |
...
| dumbproxy | `1.51.1` | `dumbproxy/Dockerfile` | 轻量 HTTP 代理 |
...
| 组件 | 版本 | 引用文件 | 说明 |
|------|------|----------|------|
| Gitea Runner | `1.0.8` | `gitea-runner-ubuntu/Dockerfile`, `gitea-runner-windows/Dockerfile` | CI/CD Runner |
| Node.js | `24.18.0` | `gitea-runner-ubuntu/Dockerfile`, `gitea-runner-windows/Dockerfile` | JavaScript 运行时 |
| kubectl | `1.36.2` | `gitea-runner-ubuntu/Dockerfile` | Kubernetes 命令行工具 |
| Helm | `4.2.3` | `gitea-runner-ubuntu/Dockerfile` | Kubernetes 包管理器 |
...
| Apache JMeter | `5.6.3` | `gitea-runner-ubuntu/modules/jmeter.sh`, `gitea-runner-ubuntu/modules/common.sh` (默认值) | 性能测试工具 |
...
| 组件 | 版本 | 引用文件 | 说明 |
|------|------|----------|------|
| Git for Windows | `2.54.0` (v2.54.0.windows.1) | `windows/Dockerfile.bak` | Git + Git Bash / MSYS2 |
| Git (Chocolatey) | latest | `windows/Dockerfile` | 通过 Chocolatey 安装 |
| PowerShell 7 | `7.4.7` | `windows/Dockerfile.bak` | PowerShell Core |
...
| VS Build Tools 2022 | 17 release | `windows/Dockerfile`, `windows/Dockerfile.bak` | Visual Studio Build Tools |
| MSVC v143 | latest (VS 2022) | `windows/Dockerfile`, `windows/Dockerfile.bak` | C/C++ 编译器工具链 |
| Windows 10 SDK | `10.0.19041` | `windows/Dockerfile.bak` | Windows SDK (19041) |
...
| PyTorch | `2.12.*` | `pytorch/install_pytorch.sh` | 深度学习框架 (CUDA 13.2) |
| torchvision | `0.27.*` | `pytorch/install_pytorch.sh` | 计算机视觉库 |
| torchcodec | `0.12.*` | `pytorch/install_pytorch.sh` | 视频编解码库 |
...
| Action | 版本 | 引用文件 | 说明 |
|--------|------|----------|------|
| `actions/checkout` | `v6` | 全部 workflow | 代码检出 |
| `docker/setup-docker-action` | `v5` | 全部 workflow | Docker 环境初始化 |
| `docker/login-action` | `v3` | 全部需登录 ghcr 的 workflow | 容器仓库登录 |
| `actions/upload-artifact` | `v7` | `issue-sync-image.yml` | 构建产物上传 |
